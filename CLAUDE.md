...
| `/fleet` | Fleet | PermRoute(can_view_fleet) | All vehicles/equipment — investment/value/equity grid, list, or table (Sold excluded by default) |
| `/fleet/maintenance` | FleetMaintenance | PermRoute(can_view_fleet) | Every maintenance task fleet-wide, grouped by task name |
...
- **`/fleet/maintenance`** (`FleetMaintenance.jsx`) — every Maintenance Item fleet-wide, **grouped by task name** rather than by asset, since the same task (e.g. "Blade sharpen/replace") tends to come due on several machines at once — one row, "Mark all done" bulk action, per-asset rows underneath. Sorted by urgency; Sold assets' items are excluded. `markDone`/`markAllDone` both go through the shared `computeMarkDoneFields()` in `lib/fleet.js` so single and bulk completion behave identically — don't reimplement the Last-Done/Next-Due/Status logic inline.
- **`Est. Market Value` means as-is/current-condition value, not "worth once fixed."** Conflating the two was a real bug — a machine could carry a healthy pre-repair estimate right up until a shop quote revealed a serious defect and it sold for a fraction of that number. `Est. Value After Repair` + `Est. Repair Cost` are separate fields (Equipment, In Repair/Down assets only), with `Repair Upside` (`Est. Value After Repair − Est. Repair Cost − Total Invested`) as a same-record Airtable formula — a same-table calc, unlike `Next Due Date`, so it didn't hit the client-computation requirement. `FleetDetail.jsx` shows this as a "Worth fixing?" card only while the asset is actually broken.
- **Money-pit flag** — `isMoneyPit()` in `lib/fleet.js` flags an asset once `Total Invested` reaches `Purchase Price` (repair spend alone has caught up to the purchase). Surfaced in both the Fleet page's Needs Attention panel and inline on the detail page. Simple threshold, deliberately not fancier — the value's in catching the pattern before another repair decision, not in modeling it precisely.
- **Sold handling** — Sold assets are excluded from the default `/fleet` view (mirrors the "Show archived" pattern already used in `SubcontractorsPanel.jsx`); a "Show sold (N)" checkbox in the filter panel brings them back, and selecting `Status = Sold` directly always shows them regardless of the toggle. `Sold Date` + `Sale Price` are separate Equipment fields (Sale Price should be **total consideration**, not just cash — a trade where a repair bill got forgiven as part of the deal still counts). `Realized Gain/Loss` (`Sale Price − Total Invested`) is a same-record Airtable formula, the realized counterpart to the unrealized `Est. Equity`.
- "Needs attention" panel (missing `Purchase Price`/`Purchase Date`, stale market value, money-pit assets, fleet-wide Overdue/Due Soon maintenance) is derived client-side, mirrors the Insurance page's `issues` pattern. Deliberately kept inside the Fleet module's own panel rather than the global Dashboard's Action Items — the Dashboard only gets one summary line on its existing Fleet widget, by design.
...
**Supabase edge functions and `pg_cron` jobs deploy separately** — they are not part of this GitHub Actions workflow. New/changed edge functions need `supabase functions deploy <name>` (some with `--no-verify-jwt`) and their secrets set via `supabase secrets set` (`AIRTABLE_PAT`, `GOOGLE_MAPS_API_KEY`, `VAPID_PUBLIC_KEY`/`VAPID_PRIVATE_KEY`/`VAPID_SUBJECT`, `DISCORD_BOT_TOKEN`/`DISCORD_PUBLIC_KEY`/`DISCORD_DIGEST_CHANNEL_ID`, `RESEND_API_KEY`, `STRIPE_HAPPY_CUTS_KEY`). `supabase db push` doesn't work in this repo — blocked by a pre-existing remote migration-history mismatch (an old `20260810`-timestamped migration recorded on the server with no matching local file) that has nothing to do with any individual change.

**New SQL migrations can be applied directly via the `run-migration` edge function** (`supabase/functions/run-migration/index.ts`) instead of pasting into the SQL editor by hand — `POST` the migration file's contents as `{"sql": "..."}` to `https://zhboqhhjijktsanxhwjv.supabase.co/functions/v1/run-migration` with an `x-migration-key` header matching the `MIGRATION_RUNNER_KEY` secret. Deployed with `--no-verify-jwt` (no Supabase user session involved — nothing user-facing calls this) and gated by that shared-secret header instead, same trust model as the existing `hermes-gateway` function's `HERMES_API_KEY`. Connects directly to Postgres via `SUPABASE_DB_URL` (already present on every edge function's runtime) using the `postgres` npm package's `unsafe()` for full multi-statement SQL, including `do $$ ... $$` blocks. Returns `{ok:true, rows:[...]}` (truncated to 200 rows) so it also works as a quick read-only query tool for checking live DB state instead of guessing from `supabase inspect db table-stats` row-count estimates.
